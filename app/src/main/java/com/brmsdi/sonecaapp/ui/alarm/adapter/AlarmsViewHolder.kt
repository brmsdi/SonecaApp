...
class AlarmsViewHolder(
    ...
    ViewHolder(row.root) {
    fun bind(alarmWithDaysOfWeek: AlarmWithDaysOfWeek, position: Int) {
        ...
        val hour =
            if (alarm.hour.toString().length == 1) "0".plus(alarm.hour.toString()) else alarm.hour.toString()
        val minutes =
            if (alarm.minutes.toString().length == 1) "0".plus(alarm.minutes.toString()) else alarm.minutes.toString()
        row.textTime.text = String.format(" %s:%s", hour, minutes)
        ...
        if (lastLocation != null) {
            currentDistance =
                locationCalcListener.calculateDistance(
                    lastLocation,
        ...
        val backgroundDrawable = when (position % 5) {
